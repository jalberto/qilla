...
func TestUnitsAgentRole(t *testing.T) {
	...
	cfg := &config.Config{
		...
		Host:     config.Host{Roles: map[string]string{host: "agent", "midori": "engine"}},
	...
	if len(u) != len(want) {
		t.Fatalf("agent units: got %v, want only %v", u, want)
	}
	for _, w := range want {
		if _, ok := u[w]; !ok {
	...
	if u := Units(cfg, Paths{ConfigDir: "/h/.config/qilla", Qilla: "/q"}); len(u) != 0 {
		t.Fatalf("agent without qmd-refresh routine must get no units: %v", u)
	}
}
