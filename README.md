...
| **session hooks** | `qilla hook session-start` opens a vault session with what changed (`qilla catchup`, in-process), a missing daily note, missing `qilla init` harness files and a qmd index older than a day (refreshed in the background); `qilla hook stop` blocks once for a `qilla:learn` pass every `[hooks] learn_every` new tool calls (default 15, 0 = off), checkpointed per session under `<state_dir>/hooks/`. Both are native, vault-only and never run under `QILLA_RUN`; `[hooks] session_start` / `stop` stay as optional user commands run after the native part, and are empty by default |
...
| `[fetch]` | `qilla fetch` ladder: `hister_url` (default `http://127.0.0.1:4433`), `ladder_url` (default `http://nasdxp:8082`), `mirrors` (host → mirror, built-in reddit → safereddit.com), `route` (`auto` = per-domain ledger + fetch-route decider, `default` = fixed order), `headed` (`never` \| `ask` (default: one Noul decision, caller `fetch-headed`, before a window opens — a page that needs a login/captcha is not opened: the rung is `needs-human`, `qilla fetch` exits 3 with "needs you in the browser" and adds a `qilla remind add` reminder) \| `always`; routines' `fetch()` is always `never` unless `[routines.<name>.fetch] headed` overrides it). The headed rung opens at most one window per fetch and closes the session if the fetch ends without content; its window class is `[browser] class` (default `qilla-browser`), which must match the niri rule `match app-id="qilla-browser"` → `open-on-workspace "qilla"`, `open-focused false`; karakeep reuses the routine setting `karakeep_url` and the `karakeep` secret |
...
| `[deciders]` | the decision layer: `conf_floor`, `policy_version`; the local route is kev: `kev_url` (default `http://127.0.0.1:8009`, `""` = the deprecated Lemonade path), `kev_model` (default `kev-latest`), optional `kev_key` secret; `default_route` (`jev` default \| `local`: where `auto` sends public input); the remote route: `jev_enabled`, `jev_url` (default `https://api.typesafe.ai`), `jev_model` (default `jev-latest`), `jev_daily_max` (default 200) — the Jev token is the `jev_key` secret, `qilla secret set jev_key < file`. `lemonade_url` / `ask_model` are deprecated (the Lemonade decider model is retired). `qilla doctor` warns when `kev_url/v1/models` does not answer |
...
| `qilla decide ask --choice a,b,c \| --score \| --noul [--question Q] (--text T \| --stdin) [--floor F] [--route auto\|local\|jev] [--public] [--caller C]` · `qilla decide ask --show-trace [--tail N]` | one typed decision from a System One server — `--route auto` (default): `--public` input goes to Jev (when `jev_enabled` and `default_route = "jev"`), everything else to kev locally; on jev 5xx/timeout/daily cap an auto ask retries once on kev (trace `fallback`); every trace line names the `endpoint` (`jev`\|`kev`\|`lemonade`): JSON `{kind, label\|value, conf, dist, route, model, ms}` on stdout. `unknown` is always in the option set and below `conf_floor` the answer *is* `unknown` — undecided, never a guess. A backend that is down or angry answers `unknown` with an `error` and exits 0; only bad arguments exit 2. Every call appends a line to `<state_dir>/deciders/decisions.jsonl` (sha1 and length, never the text). `--route jev --public` asks TypeSafe AI's System One API instead (`POST <jev_url>/v1/systemone`, one typed question, `jev_model`): same JSON shape, `route: "jev"` and the model the API answered with, plus `tokens` on the trace line. It is off unless `jev_enabled` and the `jev_key` secret are set (`qilla secret set jev_key < key.txt`), it refuses input that is not `--public`, it stops at `jev_daily_max` calls a day (`jev daily cap`), and an explicit `--route jev` never falls back to the local route: a rate limit or an HTTP error is `unknown` with `jev rate limited` / `jev http NNN` |
